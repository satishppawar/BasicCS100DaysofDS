# Day 77 - Combination Sum V

Notebook: [77-combination-sum-v.ipynb](./77-combination-sum-v.ipynb)

## Basic Notes

What is Combination Sum V?
- `Combination Sum V` is a constraint-driven DSA problem typically solved with recursion/backtracking (or DP where applicable).
- A robust solution depends on state definition, decision choices, stopping rules, and pruning conditions.

Core theory details:
- Pattern: custom variant; define rules clearly first.
- State: depends on variant (target, picks, reuse rules).
- Complexity: variant-dependent; document assumptions in notes.
